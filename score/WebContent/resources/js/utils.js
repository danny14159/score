...
window.utils = {};
/**
 * colums=[ 'attr1','attr2',format(d){return string} ]
 * index表示对应td的index 传入字符串表示取得对应属性的值 传入函数表示自定义的值 ·函数参数d表示该行js返回对象
 ...
 * 检测#{pageNumber}类似的数据进行替换 返回的数据中list为数据,分页信息新增pageCount,recordCount,
 * params.animation:boolean[default:true]
 * 如果params.url未定义，但是params.fill_data定义了，则以fill_data的内容填充表格，而不发送ajax请求
 * loadData函数要求填入表格的对象必须是二维数组或者对象数组
 * params.bottomRow:boolean[default:true] 表明底部是否显示记录数
 */
...
var loadData = function($table, params, columns, callback, fPager) {
	...
	function fillData(data) {
		if (data == null || data.length == 0) {
			...
			// 构建表格
			$table.append("<tr><td colspan=" + nths
					+ " style='text-align:center;color:#888;'>暂无数据</td></tr>");
			if ('undefined' != typeof callback && callback != null)
				return callback($table, data);
			else
				return;
		}
		...
		if(params.bottomRow !== false)
			tbody += "<tr><td colspan=" + nths
				+ " style='text-align:center;color:#888;'>共" + data.length
				+ "条记录</td></tr>";
		...
		else $table.append(tbody).hide().fadeIn(300);
	...
	if(params.fill_data){
		if(typeof params.fill_data.length == "undefined"){
			throw new TypeError('fill_data只接受数组类型的对象');
		}
		fillData(params.fill_data);
		return;
	}
	/** ajax函数的返回数据应该是一个对象数组[{...}...] */
	if(!params.url) throw new Error('url参数未设置:(');
	$
			.ajax({
				'beforeSend' : function() {
					// 销毁表格
					...
					$table
							.append('<tr><td colspan='
									+ nths
									+ ' style=\'text-align:center;color:#888;\'><img src="resources/img/onload.gif">正在加载，请稍后...</div></div></td></tr>');
				},
				'error' : function() {
					// 错误时调用的函数
...
/**
 * 使该行表格可编辑，函数在列最后一个td中，隐藏先前元素，并加入ok,cancel表示提交或者撤销操作 $tr表示要编辑的tr对象
 * columns数组对象{'0':{'prop_name','inFormat':function($td){return
 * string},'outFormat','validator'},'2':...,'4':...,...},键值对，key:td所在的索引,value:对象
 * prop_name:对应json对象的属性名称 validator:function(string value){return
 * [boolean,msg]} callback:function(post_json,$tr){}---用于点击OK时的回调函数
 */
...
var editTable = function($tr, columns, callback, fAdd) {
	...
	for ( var p in columns) {
		...
		if (typeof columns[p].inFormat == "function") {
			...
			str_append = '<input type="text" size="' + size + '" value="'
					+ $td.html() + '" class="util_input"/>';
		}
		if ($td.contents().size() == 0) {
			$td.html('<div style="display:none" class="util_wrapper"></div>');
		} else {
			$td.contents().wrap(
					'<div style="display:none" class="util_wrapper"></div>');
		}
	...
	$tr.children("td").last().contents().wrap(
			'<div style="display:none"></div>');
	$tr.children("td").last().append(str_btn);

	$tr.filter(':has(input.util_input)').css('height', 28).css('over-flow',
			'hidden');
	// 使第一个input获取焦点
	...
	$tr.find(".utils_cancel").click(function() {
		...
		for ( var p in columns) {
			...
			if (cont.size() != 0)
				...
				$td.children('div.util_wrapper').remove();
		}
			...
			.bind(
					...
					function(d) {
						...
						for ( var p in _columns) {
							...
							if (typeof _columns[p].prop_name == "string") {
								if (typeof _columns[p].outFormat == "function") {
									post_json[_columns[p].prop_name] = _columns[p]
											.outFormat(_$td);
								} else {
									post_json[_columns[p].prop_name] = _$td
											.children('input.util_input').val();
								}
							}
							_$td.children('div.util_wrapper:last').nextAll()
									.remove();
							var cont = _$td.children('div.util_wrapper')
									.contents();
							if (cont.size() != 0)
								cont.unwrap();
							else
								_$td.children('div.util_wrapper').remove();
						}
						$tr.children("td").last().find('button.utils_cancel')
								.remove();
						$tr.children("td").last().find('button.utils_save')
								.remove();
						$tr.children("td").last().children('div').contents()
								.unwrap();
...
/**
 * 弹出信息框函数 msg_html:信息文本 duration:持续时间对象{show:..,during:..fade:..}单位:ms，有默认值
...
window.utils.alert = function(msg_html, type, duration) {
	...
	window.utils.c[id] = setInterval(
			'$("#util_alert_box").stop().slideToggle(500);window.clearInterval(window.utils.c['
					+ id + ']);', 3500);
};
...
/**
 * 选中一行数据(为表格的行绑定单击事件) 无奈耦合两个css样式.currtr .selecttr $table : 标准表格格式
 * selected_color: callback($td):点击单元格事触发的事件 escape:[]避免单击事件的列
 */
...
// 实例方法
/**
 * 添加一个标签页tab:{ title:'', content:function(TabView){return [obj string]|[obj
 * jQuery]} | [obj string]|[obj jQuery], //这个函数的context为 dom a元素
 * callback:function(TabView){};//渲染完成后的回调函数,context=dom a 
 * cache:boolean
 * //标记是否缓存，如果缓存则callback函数只调用一次 }
 */
...
TabView.prototype.element = function(container) {
	...
	var _tview = this.view.empty().append(this.tabs).append(this.content)
			.clone();
	...
	this.content.css('left', this.tabWidth + 4).width(
			container.width() - _pPaddingL - _pPaddingR - this.tabWidth - 5);
...
// 设置内容
TabView.prototype.setContent = function(content) {
...
var ListItem = function(parent_id) {
	...
	this.$btn_add = $('<a class="btn_add btn btn-link">添加</a>');
...
ListItem.prototype.addItem = function(item) {
	...
	var btn = $('<a class="btn btn-link btn_del">×</a>');
	...
	$li.append(btn).appendTo(this.$ul).hide().show(300);
...
ListItem.prototype.appendTo = function() {
	//构造ListItem
	var $ul = this.$ul;
	$('<div class="ListItem">').append($ul).append(this.$btn_add).appendTo(
			$('#' + this.parent_id));
};
